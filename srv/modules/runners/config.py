...
class Util(object):
    @staticmethod
    def process_include_exclude(initial_set, include_set, exclude_set,
    ...
    @staticmethod
    def parse_bin_size_val(val, res_prefix='b', default=0):
        if val is None:
...
class SectionOpts(ConfigSection):
    ...
    def _get_opt_bin_size_val(self, key, default=0, res_prefix='b'):
        if key in self.dict:
            return Util.parse_bin_size_val(self.dict[key], res_prefix)
        return default
...
class OSDGlobalOpts(SectionOpts):
    ...
    def max_journal_partitions_per_disk(self):
        return self._get_opt_int_val('max_journal_partitions_per_disk', 6)

    def use_estimated_journal_size(self):
        return self._get_opt_bool_val('use_estimated_journal_size', False)
    ...
    def bluestore(self):
        return self._get_opt_bool_val('bluestore', False)

    def dmcrypt(self):
        return self._get_opt_bool_val('dmcrypt', False)
...
class DeviceOpts(SectionOpts):
    ...
    def use_estimated_journal_size(self):
        return self._get_opt_bool_val('use_estimated_journal_size',
                                      self.parent.use_estimated_journal_size())

    def journal_size(self, res_prefix='b'):
        return self._get_opt_bin_size_val('journal_size',
                                         self.parent.journal_size(res_prefix),
                                         res_prefix)
    ...
    def bluestore(self):
        return self._get_opt_bool_val('bluestore', self.parent.bluestore())

    def dmcrypt(self):
        return self._get_opt_bool_val('dmcrypt', self.parent.dmcrypt())
...
class HostOpts(IncludeExcludeSection, SectionOpts):
    ...
    def use_estimated_journal_size(self):
        return self._get_opt_bool_val('use_estimated_journal_size',
                                      self.globals.use_estimated_journal_size())

    def max_journal_partitions_per_disk(self):
        return self._get_opt_int_val('max_journal_partitions_per_disk',
                                     self.globals.max_journal_partitions_per_disk())

    def bluestore(self):
        return self._get_opt_bool_val('bluestore', self.globals.bluestore())

    def dmcrypt(self):
        return self._get_opt_bool_val('dmcrypt', self.globals.dmcrypt())
...
class OSDSSection(IncludeExcludeSection, ConfigSection):
    ...
    def _potential_osd_members(self):
        ...
        """
        return Util.process_include_exclude(
...
class NetworksSection(IncludeExcludeSection, ConfigSection):
    def __init__(self, config):
        super(NetworksSection, self).__init__('admins', config)
        self.network_members_initialized = False
        self.network_members_set = None

    def network_members(self):
        if not self.network_members_initialized:
            self.network_members_set = self._network_members()
            self.network_members_initialized = True
        return self.network_members_set

    def _network_members(self):
        """
        Returns:
            list(string): the networks according to the include/exclude
                          rules specified in the config file.
        """
        return Util.process_include_exclude(
                                   self.parent.members(), self.include_set(),
...
#             'max_journal_partitions_per_disk': 6,
...
# print(h.journal_size())
# print(h.device('vdc').journal_size())
# print(h.device('vdc').bluestore())
# print(h.device('sdb').bluestore())
# print(h.device('sdb').dmcrypt())
# print(h.device('sda').dmcrypt())
# m = c.mons
